...
console.log('Want to play a game? Type play()');
...
function play(config) {
    ...
    while (gameOn === true) {
        ...
        computer = prompt('Do you want to play with another person? Type yes or no') !== 'yes';
        drawBoard(round++, board);
        for (var i = 0; i < 9; i++) {
            if (solutions(board)) {
                console.log(`%cPlayer one wins!`, 'font-size: x-large');
                gameOn = false;
                break;
            }
            ...
            while (board[ask] !== null || ask === undefined) {
                if (player1Move === false && computer === true) {
                    ...
                    ask = prompt(`${currentPlayer.title} where would you like to go (1-9)? (type "exit" to leave)`);
                    if (ask == 'exit') {
                        gameOn = false;
                        break;
                    }
        ...
        if (gameOn === true) {
            console.log("%cWe tied. Good game!", "font-size: x-large");
            gameOn = false;
        }
        if (ask !== 'exit' && prompt("Would you like to play again? Type yes or no") !== "no") {
            gameOn = true;
        }
    }
}